...
**實施要點：**
1. **使用 PostgreSQL 外鍵名稱進行 JOIN** - `table!foreign_key_name (columns)`
2. **只在主表檢查 RLS** - characters 表檢查一次，子表通過 JOIN 自動關聯
3. **減少 `auth.uid()` 調用** - 從 7 次減少到 1 次
