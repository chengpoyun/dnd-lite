...
### 4. 🧪 單元測試工作原則 (必須遵守 - 最高優先級)
**🎯 確保代碼品質和防止回歸問題**

**⚠️ 強制執行規則：**
- **🚨 完成任何功能修改後，立即設計對應的單元測試** - 不得等到用戶說「此問題已解決」才開始
- **🚨 在說「已完成」或「優化完成」之前，必須先設計單元測試** - 測試是工作的一部分，不是可選項
- **🚨 修改 UI 組件時，必須測試樣式類別、互動行為、狀態管理** - 確保重構不破壞功能

**必須執行的測試檢查：**
- **所有修改都需確認單元測試沒有問題** - 每次代碼變更後都要執行 `npm test` 確保現有測試通過
- **每次成功修復一個問題，都要考慮是否新增相關的單元測試** - 為修復的 bug 建立測試案例防止再次發生
- **每次實作完畢一個新功能，立即新增相關的單元測試** - 為新功能建立完整測試覆蓋率（不是「考慮」，是「必須」）
- **用戶說「此問題已解決」時，設計簡單的單元測試** - 為確認的修復建立測試保護

**測試設計原則：**
- ✅ UI 組件測試：檢查元素存在、樣式類別、互動行為
- ✅ 狀態管理測試：驗證 useState、useEffect 的行為
- ✅ 表單驗證測試：測試空值、無效值、邊界值處理
- ✅ 資料保存測試：確認所有狀態更新都有對應的持久化機制
...
**AI 助理自我檢查清單：**
- [ ] 我修改了任何代碼嗎？→ 是 → 必須設計測試
- [ ] 我新增了 UI 組件嗎？→ 是 → 必須測試渲染和互動
- [ ] 我修改了樣式嗎？→ 是 → 必須測試 CSS 類別
- [ ] 我準備說「完成」了嗎？→ 是 → 檢查是否已設計測試
...
- **git push（🚨 絕對需要用戶明確確認）** - ⚠️ **只有在用戶明確說「此問題已解決」時才能推送到遠端**

**🚨 強制規則（最高優先級）：**
- **禁止自動推送** - AI 助理不得在完成實作後自動執行 `git push`
- **必須等待確認** - 即使功能完整、測試通過、commit 完成，仍必須等待用戶明確說「此問題已解決」
- **完成實作後的正確流程**：
  1. ✅ 完成功能實作
  2. ✅ 執行測試確保通過
  3. ✅ `git commit` 提交到本地
  4. ⏸️ **停止並報告完成狀態** - 告知用戶「已完成實作並提交到本地，等待您確認後推送」
  5. ⏸️ **等待用戶確認** - 用戶說「此問題已解決」或「可以推送」時才執行下一步
  6. ✅ `git push` 推送到遠端

**標準工作流程 - 僅在用戶說「此問題已解決」時執行：**
  1. **移除測試 log** - 搜尋並刪除所有帶 `[DEBUG]` 標記的測試日誌
  2. **🚨 設計單元測試（強制）** - 為尚未推送的修改建立核心測試保護（如果尚未設計）
  3. **執行測試** - 運行 `npm test` 確保所有測試通過
  4. **提交到本地** - `git commit` 記錄變更（如果尚未提交）
  5. **推送到遠端** - `git push` 將變更推送至遠端

**AI 助理自我檢查清單（執行 git push 前）：**
- [ ] 用戶是否明確說了「此問題已解決」或類似確認語句？
- [ ] 如果沒有，是否已停止等待用戶確認？
- [ ] 如果有確認，是否已完成所有前置步驟（移除 log、測試通過、commit）？
