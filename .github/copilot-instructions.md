...
- **git push（需確認）** - ⚠️ **只有在用戶說「此問題已解決」時才能推送到遠端**
- **標準工作流程** - 當用戶說「此問題已解決」時，按以下順序執行：
  1. **移除測試 log** - 搜尋並刪除所有帶 `[DEBUG]` 標記的測試日誌
  2. **🚨 設計單元測試（強制）** - 為尚未推送的修改建立核心測試保護（如果尚未設計）
  3. **執行測試** - 運行 `npm test` 確保所有測試通過
  4. **提交到本地** - `git commit` 記錄變更
  5. **推送到遠端** - `git push` 將變更推送至遠端（**僅在用戶確認後執行**）
- **提交前檢查** - 確認所有測試通過、代碼符合規範、已設計單元測試
