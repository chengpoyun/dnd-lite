...
- 測試：新增 `terrainRewardCatalog.test.ts`，確保獎勵表每個格子的素材名稱都存在於 MH素材目錄，日後目錄改名而獎勵表沒跟著改時會直接失敗。

## 2.6.14

- 整理：`scripts/fix-mh-material-names.mjs` 內寫死的歷史改名清單是一次性的，改成可重複使用的 `scripts/rename-mh-materials.mjs`——以命令列參數指定「舊名=新名」（可多組），預設預覽、`--apply` 才寫入，只比對完全相符的舊名，同時處理玩家道具名稱與裝備插槽快照。日後目錄改名／合併重複素材時可直接使用。`scripts/dump-mh-materials.mjs`（唯讀撈資料）保留。

## 2.6.13

- 資料：依使用者確認再合併 9 組重複素材，並把 DB 玩家持有的同名道具一併改名（共 9 筆）：`大骨堆`→`大骨殼`、`小型／中型／大型怪獸骨`→`小／中／大魔物骨`、`骨`→`骨頭`、`雪草`→`雪山草`、`麻痺菇`→`麻痺蘑菇`、`硝化蘑菇`→`爆炸菇`、`壓蟲`→`蟋蟀`。
- 資料：Royal Armor Sphere 的中文名由 `皇家鎧玉` 改為 `王鎧玉`（目錄與玩家資料，`真鎧玉` 也對應到 `王鎧玉`）。通用採集素材維持不填 CR。

## 2.6.12

- 資料：依使用者確認合併 7 組重複素材，並把 DB 中玩家持有的同名道具一併改名（`fix-mh-material-names.mjs` 新增對照，共 4 筆）：`古龍血`→`古龍的血`、`大型魔物骨`→`大魔物骨`（描述「極稀有防具升級素材。」併入）、`野獸骨`→`獸骨`、`鳥獸骨`→`鳥龍種的骨`、`小骨堆`→`小骨殼`、`尖鎧玉`→`重鎧玉`、`真鎧玉`→`皇家鎧玉`。保留的鎧玉系列為 鎧玉／上鎧玉／重鎧玉／皇家鎧玉。

## 2.6.11

- 資料：新增 `泡狐龍的毛皮` (Mizutsune Pelt，CR 11，僅護甲用)；泡狐龍其餘素材維持現有中英文名（錦鰭＝Fin、紫毛＝Purplefur）。

## 2.6.10

- 資料：依使用者提供的素材清單截圖，新增 14 筆尚未收錄的素材（先建檔，效果待補；鑲嵌用途依截圖 A/W 標記設定）——幼慘爪龍的牙／逆鱗（CR 4）、黑狼鳥的厚鱗／地獄耳／華麗的喙（CR 16）、歷戰煉黑龍的黑鱗／焦甲／燃爪／黑翼／凶眼／光玉與歷戰湧出的熔岩（CR 18）、冰人魚龍的剛爪（CR 14）、通用素材滋養萃取液；並補上使用者確認的英文名：千刃龍的飛膜+ Seregios Slavescale+、千刃龍的鏡玉 Seregios Lens、爆錘龍的耐熱殼 Uragaan Scute、歷戰煉黑龍的黑焰 T.Miralis Smelter。

## 2.6.9

- 資料：依使用者提供的各怪物資料補上素材 CR——千刃龍 15、歷戰麒麟 19、冰人魚龍 14（含凍結袋）、爆錘龍 6、天狗獸 7、幼慘爪龍 4、金獅子 15、冥淵龍 28、浮空龍 7、歷戰煉黑龍 18（黑焰、歷戰不滅的交心）、爵銀龍 20；並依素材清單補上字面明確對應的英文名（槍尾+ Seregios Impaler+、凍結袋 Cryo Sac、爆錘龍的甲殼/紅玉/骨髓、天狗獸的毛/角、幼慘爪龍的爪/鱗、金獅子的尖角/黑毛、冥淵龍的翼爪 Archdemon Wingtalon、浮空龍的寶玉 Lumu Gem）。

## 2.6.8

- 資料：`黑狼鳥復甦的喙` 與 `復甦的喙` (Rejuvenated Beak) 為同一素材，統稱 `復甦的喙`——移除目錄中的 `黑狼鳥復甦的喙`，並把 DB 玩家持有的同名道具改名（`fix-mh-material-names.mjs` 新增對照）。
- 資料：黑狼鳥素材補上 CR——普通個體（鱗／甲殼／鬃毛／翼／尾巴／耳／復甦的喙）CR 7；銀狼個體（重殼 Garuga Cortex／銀狼毛 Garuga Silverpelt／剛翼 Garuga Fellwing）CR 16，並補上重殼與銀狼毛的英文名。

## 2.6.7

- 資料：新增 `黑狼鳥的剛翼` (Garuga Fellwing)，與 `黑狼鳥的翼` (Garuga Wing) 是不同素材；兩者依使用者提供的原文補上護甲效果（對中毒狀態豁免 +2 加值／優勢）與武器效果（異常狀態攻擊強化 小 +6 秒／中 +12 秒）。
- 修正（玩家資料）：新增 `scripts/fix-mh-material-names.mjs`（預設只預覽、`--apply` 才寫入、只比對完全相符的錯誤名稱），把 DB 中打錯字／缺字的 MH素材名稱改為正確名稱：`冰人魚龍涷鱗`→`冰人魚龍的凍鱗`、`冰人魚龍重殼`→`冰人魚龍的重殼`、`冰人魚龍特上鰭`→`冰人魚龍的特上鰭`、`冰人魚龍冰玉`→`冰人魚龍的冰玉`、`溟波龍的鋼爪`→`溟波龍的剛爪`、`溟波龍的天麟`→`溟波龍的天鱗`、`爆麟龍的翼`→`爆鱗龍的翼`、`溟淵龍的翼爪`→`冥淵龍的翼爪`、`黑狼鳥的鋼翼`→`黑狼鳥的剛翼`（共 8 筆道具、5 筆裝備插槽快照，插槽順序與空插槽保留）。

## 2.6.6

- 資料：新增唯讀腳本 `scripts/dump-mh-materials.mjs`（固定 SELECT、不寫入、輸出不含使用者身分），撈出玩家持有與裝備插槽內鑲嵌的 MH素材，比對 `data/mh-materials.json` 找出玩家已填寫、目錄卻沒有的資料，並收錄：`爆錘龍的紅玉` 護甲效果、新增 `金獅子的黑毛`（閃電傷害免疫）與 `爵銀龍的殼`（疫病狀態免疫）、`古龍血` 英文名 `Elder Dragon Blood`、`溟波龍` 系列 CR 14 與 `泡狐龍` 系列 CR 11（依玩家在該怪物素材上填的稀有度，同一怪物的素材共用）。

## 2.6.5

- 資料：`data/mh-materials.json` 把打錯字的 `溟波龍的天麟` 併入正確的 `溟波龍的天鱗`（沿用其「法術豁免 DC +1，寒冷或閃電法術則 +2」武器效果與 `spellDc` 加值），並移除 `天麟` 條目。

## 2.6.4

- 資料：`data/mh-materials.json` 依截圖產生的素材效果改為完整保留原句（先前為了精簡而省略的字句與括號內英文原名補回，如「(Constitution)」「(Slow)」「(half cover)」「【技】」「現在」「從中」等，數字與單位間的空格也比照原文）；移除 4 筆確認為打錯字的重複條目（`冰人魚龍重殼`／`冰人魚龍特上鰭`／`冰人魚龍涷鱗`／`溟波龍的鋼爪`，正確名稱的條目本來就存在）。

## 2.6.3

- 資料：`data/mh-materials.json` 依使用者提供的 MH 5e 工具站截圖補齊素材效果——爆鱗龍的鱗、天狗獸的爪（含英文名 Bishaten Talon）、歷戰不滅的交心（含英文名 T.Immortal Heart）、溟波龍的剛爪的武器/護甲鑲嵌效果；冰人魚龍的特上鰭護甲效果補回英文原名 (Item Prolonger+)。純資料異動。

## 2.6.2

- 整理：上一版修好的「導覽列順序」與「滑動切換順序」兩處各自維護一份分頁清單，本質上是同一份資料重複兩次、才會不同步。抽出唯一的 `TAB_LIST`（id/label/icon），導覽列直接渲染它、滑動用的 `availableTabs` 改為從它 `map` 出 id 陣列，之後只需要改一個地方。經瀏覽器重新載入與模擬觸控滑動確認導覽列顯示與切換行為都不變。

## 2.6.1

- 修正：左右滑動切換分頁的順序（`availableTabs`）跟頂部導覽列實際顯示的分頁順序不一致——導覽列是「筆記/地圖/骰子/採集」，滑動用的陣列卻是「骰子/筆記/採集/地圖」，導致從裝備往左滑會跳到骰子而不是畫面上緊接著的筆記。改成滑動順序比照導覽列順序。經瀏覽器模擬觸控滑動（左滑裝備→筆記、右滑筆記→裝備）確認一致。

## 2.6.0

- 調整：「法術」分頁原本只有 `isSpellcaster` 判定為施法職業時才顯示（頂部導覽列與可用分頁清單各一處），改為所有角色都永遠顯示，不再依職業判斷隱藏。戰鬥頁的法術命中/法術DC顯示（`showSpellStats`）不在此次調整範圍，仍依施法者判定。

## 2.5.2

- 修正：戰鬥頁「修改 HP」彈窗把暫時生命欄位整個清空後按套用，會維持原本的數值而不是歸零——欄位為空時被當成「沒有輸入所以不變」，改成欄位整個清空視為明確輸入 0。
- 修正：長休後暫時生命沒有歸零（5E 規則：暫時生命不會因為休息恢復，長休後會直接歸零），現在長休時會一併把暫時生命重設為 0 並寫回資料庫。

## 2.5.1

- 修正：2.5.0「MH素材可更新」功能套用時漏帶英文名稱（`nameEn`），現在會跟稀有度/鑲嵌效果一樣一併比對與套用（規則同樣是只加不減：目錄英文名稱為空時不視為差異、也不會蓋掉角色已填寫的值），確認對話框新增「英文名稱」列。
- 調整：「可更新」標籤與詳情頁按鈕改為紅色（原為青色），視覺上更醒目。
- 修正：道具/能力列表卡片的標籤列（`STYLES.listCard.tags`）在手機窄螢幕上標籤過多時最右邊會被裁切而非換行——共用樣式原本對標籤外層容器設了 `shrink-0`，導致該容器在 flex-wrap 版面中以「未換行前的完整寬度」計算大小而溢出、又被卡片的 `overflow-hidden` 裁掉；改為 `min-w-0 max-w-full`（拿掉 `shrink-0`）讓容器換行後仍受父層寬度限制，標籤才能正確換到下一行。

## 2.5.0

- 新增：MH素材是取得當下從目錄複製的快照，之後目錄補上/修正效果（如這次的爆鱗龍/千刃龍系列）不會自動反映到角色已持有的道具。新增「可更新」提示：道具列表卡片與詳情頁（詳情頁按鈕位置緊鄰★收藏，取代原本沒有內容的位置）在該筆素材與目錄有差異時顯示，點擊後彈出對話框列出每個有差異欄位的「舊值→新值」，需按「確認更新」才會實際套用，避免誤觸。比對與套用規則刻意「只加不減」：目錄還沒收錄的欄位（如 CR 為 null，或某一側鑲嵌效果目錄未定義）不會被視為差異、套用時也不會蓋掉角色已自行填寫的資料，僅合併目錄「有資料」的部分。新增 `services/mhMaterialCatalog.ts` 的 `getMHMaterialUpdatePreview`/`buildMHMaterialUpdatePayload` 與 `components/MHMaterialUpdateModal.tsx`。經瀏覽器實測完整流程（列表標籤 → 詳情頁按鈕 → 確認對話框 → 套用後兩處提示消失、內容更新為目錄最新值）。
...
- 規則：熟練/專精不會與角色本身的熟練度疊加，一律取兩者較高者（技能專精(2) > 熟練(1) > 無(0)；豁免只有熟練與否兩種狀態）；多個能力/物品同時賦予同一項時也是取最高，不會疊加。
- 遷移：`character_abilities`/`character_items` 的 `stat_bonuses` JSONB 新增 `savingThrowProficiency`（豁免陣列）、`skillProficiency`（技能→1或2的對照表）兩個欄位，沿用既有 JSONB 欄位、無需新增資料庫 migration。
- 修正：角色頁點開豁免/技能的詳細彈窗（如「體質」「運動」卡片），沒有顯示能力/物品賦予的熟練/專精，導致「最終豁免」「最終總計」跟外層卡片的數字對不起來（外層正確、彈窗內是舊值）。彈窗的「無/熟練」切換刻意維持只代表角色自己的熟練（職業/背景給的）——避免使用者沒動切換就按「儲存」，把來源賦予的熟練誤存成角色自己永久擁有的熟練；改為在「加值來源」列表額外補上這筆熟練加值，讓最終數字正確。
- 修正：技能詳細彈窗切換「無/熟練/專精」時，只要「基礎值」還沒手動編輯過就會被自動覆寫成新熟練度算出的預設值，容易讓使用者搞不清楚目前顯示的到底是自己填的還是系統重算的；儲存後重新開啟同一個技能，「基礎值」還會被重新烘進當時的熟練加值（例如專精後變成把 +8 也算進基礎值裡的 10）。改成「基礎值」永遠只代表屬性調整值本身，不論是第一次開啟、重新開啟、還是互動切換，都不會把熟練/專精加值算進去；熟練/專精加值一律另外顯示成獨立的加值來源列，跟著目前選擇的熟練度即時反映，「最終總計」也會跟著即時更新。一旦手動編輯過基礎值（或原本就有覆寫值），就不再顯示這筆加值、完全以輸入的數字為準，儲存時也不會誤把系統算出的中繼值存成永久覆寫。
- 修正：豁免詳細彈窗（點開「體質」「魅力」等卡片）比照上面技能的修法——「基礎豁免」原本會在角色本身已熟練時把熟練加值烘進去（例如魅力熟練時顯示「基礎豁免 +7」而非「+3」），跟技能那邊「基礎值永遠只代表調整值本身」的原則不一致。改成「基礎豁免」一律只顯示能力調整值，熟練加值不論是角色自己熟練還是能力/物品賦予熟練，都另外顯示成獨立的「熟練加值」列，跟著「無/熟練」切換即時反映。

## 2.3.0

- 新增：道具全面顯示英文名稱與稀有度。英文名稱顯示在名稱旁的小字副標；稀有度依類別分兩種呈現——MH素材的「稀有度」其實是素材來源怪物的 CR 等級（數字無固定範圍），顯示成中性灰色的「CR {數字}」徽章，不套用色階；其他道具（裝備／藥水／雜項）沿用 D&D 標準 6 級稀有度文字（普通／非常見／稀有／非常稀有／傳說／神器），依級距上色。涵蓋道具卡片、詳情彈窗、獲得物品搜尋結果三處顯示。
- 新增：編輯／新增個人物品的表單新增「英文名稱＋稀有度」共用一列的輸入區——英文名稱是文字輸入框，稀有度則依目前類別切換輸入方式：MH素材顯示數字輸入框（CR，無範圍限制），其他類別顯示 6 級稀有度下拉選單。
- 遷移：`character_items` 新增 `name_en_override`、`rarity_override` 兩個文字欄位，用來覆寫角色自己持有的道具英文名稱／稀有度（沿用既有的 override 機制，皆為新增欄位、不影響既有資料）。

## 2.2.1

- 修正：「學習新法術」按下「學習」後沒有任何反應。原因是 `createCharacterSpell` 把「材料成分」（material）當成必填欄位，但 534 筆官方法術裡有近一半（253 筆）本來就沒有材料成分（純聲音／姿勢施法），送出後在後端被判定「所有欄位皆為必填」而失敗，畫面卻只在 console 印錯誤、完全沒有提示。這次把 material 從必填欄位移除（本來就該允許空字串），並補上 `SpellsPage` 學習失敗時的錯誤提示，之後若再失敗至少會看到明確訊息，不會像這次一樣看起來像沒反應。
...
- 修正：「獲得物品」的搜尋結果不再排除角色已經擁有的物品（依名稱比對）。已擁有的物品一樣會出現在搜尋清單中，點「獲得」時行為改成直接開啟該物品原本的詳情彈窗（等同在道具列表點擊它），可以直接調整數量，而不是靜默地被隱藏、或另外多開一筆重複的物品。
...
- 修正：MH素材的武器/護甲插槽效果，只要有寫「效果說明」文字，鑲嵌到已裝備的裝備上後就會自動出現在戰鬥頁的「其他效果」清單——不需要再額外勾選「此效果會影響角色數值」並在跳出的欄位裡把同一段文字重複貼一次。原本兩者是分開的欄位：`note`（效果說明，一定會存、只用於道具描述）與 `stat_bonuses.other`（戰鬥頁專用，要另外勾選才會出現輸入框），造成同一段效果得打兩次字。這次改成彙總邏輯優先看 `stat_bonuses.other`（若使用者刻意填了不同文字，尊重那個覆寫），沒填才退回用效果說明的文字；UI 完全沒動，「此效果會影響角色數值」的勾選框跟裡面的「其他」欄位還在，只是不再是必填。

## 1.18.1

- 修正：道具頁、能力頁的搜尋改為**永遠全域搜尋**，不再受目前選的類別／來源篩選影響——例如切到「藥水」類別後搜尋一件裝備的名稱，一樣找得到。1.18.0 剛加上搜尋欄時搜尋是跟類別/來源疊加（AND），這次拿掉那層限制。

## 1.18.0

- 新增：道具頁、能力頁比照筆記頁加上搜尋欄，位置在既有的類別／來源篩選 chip 列下方、清單上方，與該篩選同時套用（AND）。道具頁搜尋名稱＋效果說明（含 MH素材自身的插槽效果、裝備上已鑲嵌素材的效果摘要，兩者本來就會算進 `displayDescription`，不用額外處理）；能力頁搜尋名稱＋說明文字。三頁都不分大小寫，找不到結果時顯示「找不到符合的道具／能力／筆記」。
- 新增：三個搜尋欄都補上「有文字才顯示」的 x 清空按鈕，點了會清空搜尋字並回到目前篩選下的全部清單。
- 重構：新增共用元件 `components/ui/SearchInput.tsx`（外觀＋x清空邏輯）與共用函式 `utils/common.ts` 的 `matchesSearch(query, ...fields)`（大小寫不分的比對邏輯），筆記／道具／能力頁改叫這兩個共用件，不再各自重寫一份。

## 1.17.1

- 資料：移除「資訊」分頁全部 5 份**帳號限定文件**（創角種族工具、地區資源採集表(玩家)、道具列表+地區採集、魔物獵人武器稀有度説明、魔物獵人道具圖鑑）。這些內容已經公開發布到 GitHub Pages，不需要再用 DB 授權名單控管。`info_documents` 的資料表與程式碼**保留**，之後還要放帳號限定文件時可以直接再塞。
- 新增：預設連結多一筆「**TRPG 跑團工具入口網**」（`https://woofname.github.io/Magelan/`），取代上面那 5 份文件的入口。`services/infoLinks.ts` 的預設值從單筆改成 `DEFAULT_INFO_LINKS` 陣列，新帳號第一次開資訊頁時整組補上；既有的 4 個帳號（2 個登入 + 2 個匿名）另外用一次性 SQL 補資料。
- 預設連結只在帳號「完全沒有連結」時補，所以使用者刪掉之後不會又被自動塞回來 —— 這是刻意不做自動補齊的原因。

## 1.17.0

- 新增：**組織與聲望系統**。角色可以不加入組織，也可以加入多個；每個組織各自累積聲望，聲望決定階級（階級 = 達到或超過的門檻個數，聲望無上限），階級決定採集素材的倍數。角色頁新增「組織」區塊（放在冒險紀錄**上方**，因為冒險紀錄的自訂項目會無限增加），顯示各組織的聲望、階級、倍數與距下一階的差距，底部顯示目前的採集倍數。
- 五個組織：獵人公會（4 階，1/10/25/50）、皇家古生物學院（7 階，1/3/10/25/50/80/100）、塔龍協會（3 階，1/10/25）、威卡德米學院（5 階，1/3/10/25/50）、螺旋委員會（5 階，1/10/25/50/100）。門檻依 DM 確認為準。倍數為 `2^(階級-1)`，唯獨**塔龍協會是 `2^階級`**（同階級強一倍，刻意設計）；**階級 0 一律 ×1**（否則公式會算出 0.5）。加入多個組織時取倍數最高者。
- 新增：地形採集的素材數量會直接乘上倍數 —— 畫面顯示與寫入道具欄的都是計算後的數字。
- 新增：手動新增 MH素材 時補上「數量」欄位與「套用組織倍數」開關（**預設勾選**，因為手動新增的素材多半也是採集來的；隊友分的素材可以關掉）。原本這個表單根本沒有數量欄位、寫死只加 1 個。兩者只在類別為 MH素材 且有倍數時顯示。
- 移除：從未被渲染過的死欄位 `extra_data.prestige`（`org`／`level`／`rankName`），它正是這次功能的前身。
- 資料同步：`organizations` 同時接進三個地方 —— `updateExtraData` 白名單、`buildCharacterStats` 讀回、`INITIAL_STATS` 與型別。已實測「加入組織 → 改聲望 → 重新整理 → 值還在」，並補了白名單專屬測試（漏掉白名單就會靜默丟棄、漏掉讀回就會 reload 後不見）。

## 1.16.1

- 資料：依最新的「地區資源採集表(玩家) 1.2Ver」更新 `data/terrain-rewards.json`，共 43 處。多數是譯名更新（硝化蘑菇→**爆炸菇**、蟋蟀→**壓蟲**、小骨殼→**小骨堆**、鳥龍種的骨→**鳥獸骨**、獸骨→**野獸骨**），另有青翠丘陵「進階」5 個類別的備用技能 DC 調整（骸骨 14→11、昆蟲 15→10、礦石 16→14、蘑菇 15→10、植物 15→10）。
- 資料：同步更新來源文件 `data/field-selection-bonus.md`（43 處：38 個儲存格 + 5 個備用 DC）。**已用 `scripts/parse-terrain-rewards.mjs` 全流程驗證**：md → parser → JSON 的輸出與本次寫入的 JSON 完全一致，之後重跑 parser 不會把這次的更新蓋掉（1.11.2 曾發生過這個問題）。
- 巨人密林「高階」蘑菇 Y=5 維持「興奮蘑菇 x 2」。來源 HTML 1.2Ver 的中文寫成「興奮蘑菇」但英文仍是 `Exciteshroom x2`，經確認為原始檔筆誤，**以英文為準**保留數量。**原始 HTML（`地區資源採集表(玩家)1.2Ver.html`）尚未修正**，下次再依它更新時要留意這一格會再被改掉。

## 1.16.0

- 文件：`CLAUDE.md` 補上「開發固定測試角色的資料改了就改了，不用還原」。原本只寫了這個角色存在與怎麼跳過去，沒說改動後要不要復原，導致驗證完會多花步驟把數值改回去。使用者的角色「貝瑞」仍然絕對不要碰。

- 新增：道具詳情彈窗的數量可以直接輸入數字，不必再一直按 − / +（要從 1 改成 50 原本得按 49 次）。失焦或按 Enter 才寫入資料庫，打字過程不會每個字元都打一次 API；輸入空白、非數字或負數會還原成目前數量且不寫入；數值沒變也不寫入。− / + 按鈕維持原樣。
...
- 新增：道具可加入「★」收藏列表——每個道具詳情彈窗新增 ☆/★ 切換鈕；道具頁新增「★」分類 tab（預設進入道具頁即顯示此分類），道具卡片上會在名稱左側顯示 ★ 標記。
- 修正：拖曳排序原本在同一畫面內一次只能移動一格，無法直接從最上面拖到最下面；改用 dnd-kit 官方建議的 `closestCenter` 碰撞判斷後已可一次拖過多個項目。

## 1.12.0

- 新增：裝備（武器／護甲）新增「鑲嵌插槽」系統，仿魔物獵人武器稀有度規則、插槽數 0~5。MH素材可分別標記為可鑲入武器／護甲插槽；鑲嵌時可當場設定素材效果（文字說明＋選填數值加成），效果僅在裝備穿戴中生效並會顯示在「加值來源」列表。鑲嵌會消耗該素材；已鑲嵌的插槽可隨時查看、編輯效果或移除（移除與編輯效果皆有相應提示）。
- 改進：道具詳情頁的插槽改為一列一個，圖示後方直接顯示素材名稱（空插槽顯示「（空）」）；道具列表新增可鑲入武器／護甲插槽的 ⚔️／🛡️ 標籤，並統一各類別標籤配色（MH素材與鑲嵌用途標籤同色系，裝備／藥水／雜項／魔法各自使用不同顏色）。
- 修正：`ItemDetailModal`／`AbilityDetailModal`／`SpellDetailModal` 這三個詳情 modal 的內容區塊少了頂部間距，導致外層 modal 殼與內層卡片在最上方直接貼合、看起來像兩個框重疊；現已補上與左右一致的間距。

## 1.11.6

- 修正：筆記頁點進單篇筆記後，改成固定貼合可視高度，輸入框以外的區域不會再把整頁往下拖動捲走（原本長筆記會讓整頁跟著捲，返回鍵容易滑出畫面外）。
- 改進：分頁順序調整，「裝備」右邊改為 筆記、地圖、骰子、地形、資訊；「地形」分頁更名為「採集」。
...
- 新增：「資訊」分頁支援**本機文件**，跟外部連結混合顯示在同一份清單（📄 圖示區分）。內容整份存進 DB（不進 git、不部署成靜態檔案，避免公開 repo／GitHub Pages 曝光），只有登入帳號才看得到，不支援匿名模式。多人共用設計：新增 `info_document_access` 授權名單表（文件 x 帳號多對多），之後要開放新帳號存取只要在授權表加一筆即可，不用複製內容；目前僅唯讀，無上傳/編輯 UI，新增文件由維運者直接寫入資料庫。點卡片一樣是開新分頁（用 blob 網址開啟抓回來的 HTML 內容）。

## 1.11.0

- 新增：**資訊**分頁（🔗），存放參考連結（如規則查詢網站），帳號層級（同一登入/匿名身分下所有角色共用，不綁特定角色）。可新增/編輯/刪除，只需標題與網址兩個欄位；點卡片以新分頁開啟連結（`target="_blank"`），不影響目前視窗。新增 `info_links` 資料表，雙軌 `user_id`/`anonymous_id` 設計與 RLS policy 比照 `characters` 表，登入與匿名模式皆可使用。帳號目前一筆連結都沒有時會自動補上預設的「異常狀態說明」（連到 5etools 的異常狀態頁面）；若之後把清單清空，下次載入會再次補上這筆預設值（未另外記錄「是否已初始化過」的旗標）。

## 1.10.2

- 改進：道具/法術/能力的詳情、編輯與新增彈窗重新排版，拿掉冗餘標題與提示文字、相關欄位合併同一列，敘述欄位改用可自動伸縮高度的元件，手機小螢幕能顯示更多內容。
- 改進：法術/道具/能力描述支援單一換行顯示（原本 Markdown 規則會把單一換行吃掉、擠成同一段）。
- 修正：戰鬥頁（先攻/攻擊命中/法術命中）與地形獎勵擲骰明細，數值為負時會顯示成「+-3」這種雙重符號。
- 修正：攻擊命中/攻擊傷害/法術命中/法術DC/AC/先攻/速度/最大HP 的編輯彈窗，切換屬性（力量/敏捷等）或修改基礎值時，「最終總計」沒有即時重新計算。

## 1.10.1

- 修正：非遊蕩者角色的戰鬥頁也會出現「偷襲傷害 1/1」的假職業資源卡片。原因是「偷襲傷害」的範本（`default_combat_actions`）誤把 `max_uses` 設為 1，導致尚未被同步過的角色（包含所有非遊蕩者角色）在合併預設項目時直接顯示範本值；法術位範本的 `max_uses` 本來就固定為 0，作為「尚未同步不顯示」的哨兵值，這次沒有比照辦理。已修正：範本 `max_uses` 改回 0，`CombatItemService.syncSneakAttackResource` 寫入角色列時改為寫死 1（不再讀範本值）。

## 1.10.0

- 新增：**地圖**分頁（📍），瀏覽科瓦雷世界地圖圖片，支援拖曳平移、滾輪／雙指縮放、雙擊縮放，右下角有「重置視圖」按鈕可一鍵回到初始置中/縮放。用套件 `react-zoom-pan-pinch` 實作（專案目前唯一為此新增的外部依賴），圖片存為靜態檔案 `public/maps/kovar-map.jpg`，不進資料庫。App 原本在主要內容區有「左右滑動切換分頁」的手勢，地圖頁內部會攔截 touch 事件冒泡，拖曳地圖不會誤觸切換分頁。

## 1.9.0

- 新增：遊蕩者的**偷襲傷害**職業資源。戰鬥頁「職業資源」比照法術位做法，依角色目前的遊蕩者等級（多職時只算遊蕩者職業，不含其他職業）自動同步一筆「偷襲傷害 XdY」項目；比照「攻擊」「疾走」等動作，每回合限用一次（`max_uses` 固定為 1，`recovery_type` 為 `turn`，換下一回合自動重置），骰數（d6 顆數）依 D&D 5E 官方表格查表決定並直接寫入項目名稱，不使用 `Math.ceil(等級/2)` 這類公式推算。查表邏輯獨立於新檔 `utils/sneakAttack.ts`（索引 0～20 對應等級的骰數陣列），與既有 `utils/spellSlots.ts` 的 `FULL_CASTER_SPELL_SLOTS` 查表風格一致；確認法術位的環位數量本來就已是查表（`getSpellSlotsForCasterLevel`），僅合併多職業等級為單一「施法者等級」的 `calculateCasterLevelForSpellSlots` 因官方規則本身即為多輸入公式（半/1/3 施法者取捨去、奇械師取進位），不適用單一查表因此維持原樣。尚未有遊蕩者等級時不建立此項目；已建立後若遊蕩者等級歸零（如移除該職業），項目會保留但骰數顯示為 0d6（比照法術位環位歸零時的既有慣例，不刪除項目）。
