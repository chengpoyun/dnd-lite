...
- 修正：技能詳細彈窗切換「無/熟練/專精」時，只要「基礎值」還沒手動編輯過就會被自動覆寫成新熟練度算出的預設值，容易讓使用者搞不清楚目前顯示的到底是自己填的還是系統重算的。改成切換熟練度不會再自動改動「基礎值」，改為在加值來源列表即時顯示一筆「熟練加值/專精加值」（正負皆可，反映切換前後的差額），「最終總計」也會跟著即時更新；一旦手動編輯過基礎值（或原本就有覆寫值），就不再顯示這筆加值、完全以輸入的數字為準，儲存時也不會誤把系統算出的中繼值存成永久覆寫。
