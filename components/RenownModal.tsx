...
interface RenownModalProps {
  ...
  currentUsed: number;
  currentTotal: number;
  onApply: (used: number, total: number) => void | Promise<void>;
}
...
export default function RenownModal({
  ...
  const handleApply = async () => {
    if (!usedResult.isValid || !totalResult.isValid) return;
    setIsSubmitting(true);
    try {
      await Promise.resolve(onApply(usedPreview, totalPreview));
    } finally {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="名聲" size="xs" disableBackdropClose={isSubmitting}>
      <div className={`${MODAL_CONTAINER_CLASS} relative`}>
        ...
        <div className={MODAL_FOOTER_BUTTONS_CLASS}>
          ...
          <ModalSaveButton type="button" onClick={handleApply} loading={isSubmitting} className={MODAL_BUTTON_APPLY_AMBER_CLASS}>
            儲存
          </ModalSaveButton>
        </div>
      </div>
